  ...
  return <article className="product-card">
    ...
    <div className="product-info"><span className="eyebrow">{product.shop?.name ?? product.category}</span><Link href={`/product/${product.id}`} prefetch={false}><h3>{product.name}</h3></Link>
      ...
      <div className="product-bottom"><Price value={product.price} oldValue={product.oldPrice}/>{cartItem ? <div className="product-cart-stepper" data-testid="product-card-stepper">
        <button disabled={cart.loading} onClick={() => cartItem.quantity === 1 ? cart.remove(cartItem.id) : cart.update(cartItem.id, cartItem.quantity - 1)} aria-label="Kamaytirish"><Minus/></button>
        <b aria-label={`${cartItem.quantity} ta`}>{cartItem.quantity}</b>
        <button disabled={cart.loading || (variant?.stock !== undefined && cartItem.quantity >= variant.stock)} onClick={() => cart.update(cartItem.id, cartItem.quantity + 1)} aria-label="Ko‘paytirish"><Plus/></button>
      </div> : <button data-testid="product-card-add" disabled={cart.loading || !variant} onClick={() => cart.add({ product, quantity: 1, color: variant?.color ?? product.colors[0], variantId: variant?.id })} aria-label={variant ? "Savatchaga qo‘shish" : "Mahsulot varianti mavjud emas"}><Plus/></button>}</div>
    </div>
  </article>;
});
