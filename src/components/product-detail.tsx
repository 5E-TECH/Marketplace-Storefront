  ...
  return <>
    <div className="detail-layout">
      ...
      <section className="detail-summary">
        ...
        <p className="stock-status" data-available={available}>{stockText}</p>
        ...
        <div className="purchase-card">
          ...
          <div className="purchase-actions"><div className="quantity"><button type="button" disabled={quantity <= 1} onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Kamaytirish"><Minus/></button><b>{quantity}</b><button type="button" disabled={!available || (selectedVariant?.stock !== undefined && quantity >= selectedVariant.stock)} onClick={() => setQuantity(quantity + 1)} aria-label="Ko‘paytirish"><Plus/></button></div><Button loading={cart.loading} disabled={!available} onClick={addToCart}><ShoppingBag/> {!selectedVariant ? "Variant mavjud emas" : !available ? "Sotuvda yo‘q" : "Savatchaga qo‘shish"}</Button><button className={`detail-heart ${favorites.has(product.id) ? "active" : ""}`} onClick={() => favorites.toggle(product)} aria-label={favorites.has(product.id) ? "Sevimlilardan olib tashlash" : "Sevimlilarga qo‘shish"}><Heart fill={favorites.has(product.id) ? "currentColor" : "none"}/></button></div>
