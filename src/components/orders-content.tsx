...
export function OrdersContent() {
  ...
  useEffect(() => { orderService.listForCurrentBuyer().then((result) => { setOrders(result.orders); setError(result.error ?? ""); }); }, []);
  if (!orders) return <div className="orders-loading"/>;
  if (!orders.length) return <section className="page-empty"><span><Package/></span><h1>Buyurtmalar hali yo‘q</h1><p>{error ? "Buyurtmalarni serverdan yuklab bo‘lmadi. Keyinroq qayta urinib ko‘ring." : "Birinchi buyurtmangiz shu yerda ko‘rinadi."}</p>{error && <p role="alert">{error}</p>}<Link className="button button--primary" href="/#products">Xarid qilish</Link></section>;
