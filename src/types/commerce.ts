...
export type TrackingPayment = { status: PaymentStatus; provider?: PaymentProvider; amount?: number; failureReason?: string };
export type OrderTracking = { orderId: string; status: OrderStatus; estimatedDeliveryAt?: string; updatedAt?: string; packages: TrackingPackage[]; payment?: TrackingPayment };
export type PaymentMethod = "cod" | "payme" | "click";
export type Order = { id: string; createdAt: string; status: OrderStatus; customer: Customer; items: CartItem[]; subtotal: number; delivery: number; total: number; payment: "cash" | "card"; paymentProvider?: PaymentProvider; paymentStatus?: PaymentStatus; warning?: string };
