...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const socialDescription = `${formatPrice(product.price)} so‘m — ${product.description}`;
...
export default async function ProductPage({ params, searchParams }: Props) {
  ...
  const structuredData = { "@context": "https://schema.org", "@type": "Product", name: product.name, description: product.description, image: product.images.length ? product.images.map(absoluteUrl) : [absoluteUrl(product.image)], sku: String(product.id), category: product.categoryInfo?.name ?? product.category, brand: { "@type": "Brand", name: product.shop?.name ?? "Elchi Market" }, ...(reviews.total > 0 && !reviews.error ? { aggregateRating: { "@type": "AggregateRating", ratingValue: reviews.rating, reviewCount: reviews.total } } : {}), offers: { "@type": "Offer", url: absoluteUrl(`/product/${encodeURIComponent(id)}`), priceCurrency: "UZS", price: product.price, availability: `https://schema.org/${available ? "InStock" : "OutOfStock"}`, itemCondition: "https://schema.org/NewCondition", seller: { "@type": "Organization", name: product.shop?.name ?? "Elchi Market" } } };
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(structuredData) }}/><Container><div className="detail-navigation"><BackButton/><nav className="breadcrumbs" aria-label="Sahifa yo‘li"><Link href="/">Bosh sahifa</Link><span>/</span><Link href="/#products">{product.category}</Link><span>/</span><b>{product.name}</b></nav></div><ProductDetail product={product} reviews={reviews} reviewDemo={env.useMockData && String(product.id).startsWith("demo-")}/><section className="content-section detail-related"><SectionHeader title="Sizga yoqishi mumkin"/><div className="products-grid products-grid--related">{similar.map((item) => <ProductCard product={item} key={item.id}/>)}</div></section></Container></main>;
}
